...
The service exposes ADB on `5555` and persists the disposable `android-home/` directory at the image's documented `/data` path; that directory is ignored by Git. This is an emulator/test device, not the physical-phone STS client. The bridge is explicitly disabled by default; when enabled later, its endpoint is `android:5555` on the Compose network. The future bridge allowlist is documented in `goop/ANDROID_BRIDGE.json`, and `goop/android_bridge.py` provides a dry-run planner plus an injected-runner execution seam. Production execution must use `shell=False`, a timeout of at most 15 seconds, and the 10 MB output cap. The bridge permits one user-approved screenshot, tap, swipe, or bounded text action per request; no raw ADB, shell, install, file transfer, port forwarding, root, or credential access.
